...
class FacultyWorkload(Base):
    ...
    user_login_id: Mapped[Optional[uuid.UUID]] = mapped_column(
    ...
    division: Mapped[Optional[str]] = mapped_column(String(8), nullable=True)
    ...
    user: Mapped[Optional["UserLogin"]] = relationship("UserLogin", back_populates="workloads")
...
class FacultyMember(Base):
    ...
    weekly_hours_total: Mapped[Optional[int]] = mapped_column(SmallInteger, nullable=True)
    user_login_id: Mapped[Optional[uuid.UUID]] = mapped_column(
        UUID(as_uuid=True), ForeignKey("user_login.id", ondelete="SET NULL"), nullable=True
    )
    chaired_programme_id: Mapped[Optional[uuid.UUID]] = mapped_column(
...
class TimetableSlot(Base):
    ...
    division: Mapped[Optional[str]] = mapped_column(String(4), nullable=True)
    ...
    subject_code: Mapped[Optional[str]] = mapped_column(String(32), nullable=True)
    subject_id: Mapped[Optional[uuid.UUID]] = mapped_column(
        UUID(as_uuid=True), ForeignKey("subjects.id", ondelete="SET NULL"), nullable=True
    )
    faculty_code: Mapped[Optional[str]] = mapped_column(String(16), nullable=True)
    ...
    venue_label: Mapped[Optional[str]] = mapped_column(String(32), nullable=True)
    venue_id: Mapped[Optional[uuid.UUID]] = mapped_column(
        UUID(as_uuid=True), ForeignKey("venues.id", ondelete="SET NULL"), nullable=True
    )
    batch: Mapped[Optional[str]] = mapped_column(String(8), nullable=True)
